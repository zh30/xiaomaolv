...
`xiaomaolv` 是一个 Rust AI gateway service，负责把 Telegram/HTTP 消息路由到 AI provider，同时管理记忆、MCP 工具、Skills、Code Mode 和可观测的 agent harness。
...
- `TELEGRAM_ADMIN_USER_IDS`：私聊访问、`/mcp`、`/skills` 管理员白名单。
...
- `src/harness/`：trajectory、observability、verification、context compaction。
...
1. 先看 `docs/agent-harness-eval.md` 和 `docs/plans/2026-06-03-agent-harness-prioritized-roadmap.md`。
